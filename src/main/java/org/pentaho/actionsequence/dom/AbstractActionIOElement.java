...
import static org.pentaho.actionsequence.dom.IActionSequenceDocument.MAPPING_NAME;
...
public abstract class AbstractActionIOElement extends AbstractIOElement implements IActionIOElement {
    ...
    /**
     * @return the name to which the input/output is mapped. Returns an empty
     * string if the input/output is not mapped.
     */
    @Override
    ...
    public void setMapping(String mapping) {
        if ((mapping == null) || (mapping.trim().length() == 0) || mapping.trim().equals(getName())) {
            if (ioElement.attribute(MAPPING_NAME) != null) {
                ioElement.addAttribute(MAPPING_NAME, null);
                fireIoChanged(this);
            }
        } else {
            mapping = mapping.trim();
            if (!mapping.equals(ioElement.attributeValue(MAPPING_NAME))) {
                ioElement.addAttribute(MAPPING_NAME, mapping);
                fireIoChanged(this);
            }
        }
    }
    ...
    public IActionDefinition getActionDefinition() {
        ...
        if (ioElement != null) {
            ...
            if (ancestorElement != null) {
                ...
                if ((ancestorElement != null)
                        && ancestorElement.getName().equals(ACTION_DEFINITION_NAME)) {
