...
import static org.pentaho.actionsequence.dom.IActionSequenceDocument.INPUT_STREAM_TYPE;
...
public class PentahoReportAction extends ActionDefinition {
    ...
    public void setReportDefinition(IActionInputSource value) {
        if (value instanceof IActionInputVariable) {
            ...
            if (variable.getType().equals(INPUT_STREAM_TYPE)) {
    ...
    public List<IActionInput> getReportParameters() {
        ArrayList<String> knownInputs = new ArrayList<>(asList(EXPECTED_INPUTS));
        ArrayList<IActionInput> reportParameters = new ArrayList<>();
    ...
    public IActionOutput setOutputReport(String publicOutputName) {
        ...
        if ((publicOutputName != null) && (publicOutputName.trim().length() > 0)) {
            ...
            for (IActionOutput actionOutput1 : actionOutputs) {
                if (actionOutput1.getType().equals(CONTENT_TYPE) && !actionOutput1.getName().equals(REPORT_OUTPUT_ELEMENT)) {
                    actionOutput1.delete();
                }
            }
        }
        return actionOutput;
