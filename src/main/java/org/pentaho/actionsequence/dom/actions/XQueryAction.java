...
public class XQueryAction extends ActionDefinition {
    ...
    public static boolean accepts(Element element) {
        return ActionDefinition.accepts(element) && hasComponentName(element, COMPONENT_NAME)
                && ((element.selectSingleNode(COMPONENT_DEF_NAME + "/" + QUERY_ELEMENT) != null) //$NON-NLS-1$
                || (element.selectSingleNode(ACTION_INPUTS_NAME + "/" + QUERY_ELEMENT) != null)); //$NON-NLS-1$;
    }
    ...
    public String[] getReservedOutputNames() {
        ...
        if (getOutput(expectedOutput) == null) {
            IActionOutput[] actionOutputs = getOutputs(RESULTSET_TYPE);
    ...
    public void setOutputResultSet(String publicOutputName) {
        setOutput(QUERY_RESULT_ELEMENT, publicOutputName, RESULTSET_TYPE);
    ...
    public IActionOutput getOutputResultSet() {
        ...
        if (actionOutput == null) {
            ...
            for (IActionOutput output : outputs) {
                if (!output.getName().equals(PREPARED_COMPONENT_ELEMENT)) {
                    actionOutput = output;
                    break;
                }
            }
    ...
    public void setOutputPreparedStatement(String publicOutputName) {
        setOutput(PREPARED_COMPONENT_ELEMENT, publicOutputName, XQUERY_TYPE);
        if ((publicOutputName != null) && (publicOutputName.trim().length() > 0)) {
            setOutputResultSet(null);
            IActionOutput[] actionOutputs = getOutputs();
            for (IActionOutput actionOutput : actionOutputs) {
                if (!actionOutput.getType().equals(XQUERY_TYPE)) {
                    actionOutput.delete();
                }
            }
        }
    ...
    public IActionSequenceValidationError[] validate() {
        ...
        if (validationError != null) {
            if (validationError.errorCode == INPUT_MISSING) {
